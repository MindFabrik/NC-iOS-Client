...
class NCCollectionViewCommon: UIViewController, UIGestureRecognizerDelegate, UISearchResultsUpdating, UISearchControllerDelegate, UISearchBarDelegate, NCListCellDelegate, NCGridCellDelegate, NCSectionHeaderMenuDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate, UIAdaptivePresentationControllerDelegate  {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        CCUtility.setDisableCrashservice(true);
        if (NCBrandOptions.sharedInstance.mindfabrik_force_lockfeature) {
            if (CCUtility.getPasscode()?.count == 0) || (CCUtility.getPasscode() == nil)
            {
                ...
                let alert = UIAlertController(title: NSLocalizedString("_mf_pin_requiered_title_", comment: ""), message: NSLocalizedString("_mf_pin_requiered_text_", comment: ""), preferredStyle: UIAlertController.Style.alert)
                alert.addAction(UIAlertAction(title: NSLocalizedString("_mf_pin_requiered_button_", comment: ""), style: UIAlertAction.Style.default, handler: {_ in
