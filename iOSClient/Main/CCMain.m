...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // MindFabrik-Change-Start
    // -------------------------------------------------------------------------------------------------
    // FUNCTION: Check if PIN is configured; when not forward to LOCK Settings
    // -------------------------------------------------------------------------------------------------
    
     if ([NCBrandOptions sharedInstance].mindfabrik_force_lockfeature) {
         
         NSString *cBlockCode = [CCUtility getBlockCode] ;
         if  (cBlockCode.length >= 1){
            // Lock enabled
            NSLog(@"[MFLOG] Lock Mode is enabled");
        
        } else {
            // Lock not configured
            NSLog(@"[MFLOG] Lock Mode not configured, enforce configuration.");
        
            // Display Message
            UIAlertController * alert = [UIAlertController
                                         alertControllerWithTitle:NSLocalizedString(@"_mf_pin_requiered_title_", nil)
                                         message:NSLocalizedString(@"_mf_pin_requiered_text_", nil)
                                         preferredStyle:UIAlertControllerStyleAlert];
            
            
            
            UIAlertAction* yesButton = [UIAlertAction
                                        actionWithTitle:NSLocalizedString(@"_mf_pin_requiered_button_", nil)
                                        style:UIAlertActionStyleDefault
                                        handler:^(UIAlertAction * action) {
                                            
                                            //Set Global Variable PIN Status Variable
                                            mfSetPINStatus = 1;
                                            
                                            // Show More
                                            [self.parentViewController.tabBarController setSelectedIndex:k_tabBarApplicationIndexMore];
